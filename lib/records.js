...
(function (root) {
   ...
   *   onRemoved(record, restore)  optional; enables swiping a row aside to
   *                          remove it. Called after the removal with a
   *                          function that puts the record back where it was.
   */
  function create(options) {
    ...
    function paint() {
      ...
      rows.forEach(function (item) {
        ...
        if (options.onRemoved) {
          row.insertBefore(el('span', 'rc-row-hint', 'Remove'), card);
    ...
    /** Takes a record out, and hands back the way to put it back exactly where
        it was - which is what makes removing by gesture safe. */
