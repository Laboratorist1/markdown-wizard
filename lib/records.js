...
(function (root) {
   ...
   *   onRemoved(record, restore)  optional; called after a record is deleted
   *                          from the document, with a function that puts it
   *                          back where it was.
   *   onHidden(record, restore)  optional; enables swiping a row aside to take
   *                          it out of the view. This does not touch the
   *                          document - the record is only hidden from this
   *                          list - and restore brings it back.
   */
  function create(options) {
    ...
    // Rows taken out of the view by a swipe. The document is untouched: this is
    // the list being tidied, not the data being changed.
    var hidden = [];
    ...
    function visible() {
      ...
      var rows = records.filter(function (item) {
        if (hidden.indexOf(item) !== -1) return false;
    ...
    function paint() {
      ...
      var showing = records.length - hidden.length;
      count.appendChild(document.createTextNode(
        rows.length === showing && !state.query
          ? records.length + ' records · ' + fields.length + ' fields'
          : rows.length + ' of ' + records.length + ' records'));

      if (hidden.length) {
        count.appendChild(document.createTextNode(' · ' + hidden.length + ' hidden '));
        var showAll = el('button', 'rc-show-all', 'Show all');
        showAll.type = 'button';
        showAll.addEventListener('click', function () {
          hidden = [];
          paint();
        });
        count.appendChild(showAll);
      }
      ...
      rows.forEach(function (item) {
        ...
        if (options.onHidden) {
          row.insertBefore(el('span', 'rc-row-hint'), card);
          makeSwipeable(row, card, item);
        }
    ...
    /** Takes a row out of the view only. The document is not touched.
        Hiding lasts as long as the document is open. */
    function hideRecord(item) {
      if (hidden.indexOf(item) === -1) hidden.push(item);
      paint();
      return function restore() {
        var at = hidden.indexOf(item);
        if (at !== -1) hidden.splice(at, 1);
        paint();
      };
    }

    /** Deletes a record from the document, and hands back the way to put it
    ...
    function removeRecord(item) {
      ...
      return function restore() {
        collection.items.splice(index, 0, item);
        records = collection.items.filter(isRecord);
        paint();
        if (options.onChange) options.onChange();
      };
      ...
      if (options.onDelete) {
        ...
        remove.addEventListener('click', function () {
          options.onDelete(item).then(function (confirmed) {
            if (!confirmed) return;
            var restore = removeRecord(item);
            closeRecord();
            if (restore && options.onRemoved) options.onRemoved(item, restore);
          });
        });
