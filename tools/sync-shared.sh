...
set -e
cd "$(dirname "$0")/.."
for file in markdown.js markdown.css structured.js structured.css book.js book.css; do
  cp "shared/$file" "lib/$file"
  cp "shared/$file" "extension/src/lib/$file"
done
echo "synced shared/{markdown,structured,book}.{js,css} into lib/ and extension/src/lib/"
